// search/fuzzyMatch.ts
//
// Verbatim port of frappe/public/js/frappe/ui/toolbar/fuzzy_match.js — a
// desk.bundle file that never loads in the Vue shell. Scoring must stay
// identical to the legacy awesomebar's, since `searchUtils.ts` ranks every
// result type off these numbers.
//
// LICENSE
...
// CONTRIBUTORS Jørgen Tjernø (async helper), Anurag Awasthi (0.2.0)
...
const LEADING_LETTER_PENALTY = -5 // penalty for every letter in str before the first match
const MAX_LEADING_LETTER_PENALTY = -15 // maximum penalty for leading letters
const UNMATCHED_LETTER_PENALTY = -1

export type FuzzyResult = [boolean, number, number[]]

/**
 * Does a fuzzy search to find `pattern` inside `str`.
 * Returns [matched, score, positional match indices].
 */
export function fuzzy_match(pattern: string, str: string): FuzzyResult {
...
function fuzzy_match_recursive(
	pattern: string,
	str: string,
	pattern_cur_index: number,
	str_curr_index: number,
	src_matches: number[] | null,
	matches: number[],
	max_matches: number,
	next_match: number,
	...
	let best_recursive_matches: number[] = []
