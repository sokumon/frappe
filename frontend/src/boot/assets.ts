/* Ported verbatim from frappe/public/js/frappe/assets.js (a desk.bundle file from
 * the removed "framework bundle"). Wrapped in an install fn. Already modern JS
 * (no jQuery, no imports); only frappe.dom.freeze/unfreeze + frappe.boot are used
 * at runtime. Defines frappe.require + frappe.assets (AssetManager: lazy js/css
 * loading, localStorage cache eviction, bundled_asset resolution).
 *
 * Consumed by frappeApp (clear/init_local_storage), dom.create_shadow_element,
 * microtemplate, toolbar, utils (frappe.require lazy bundles) — so install before
 * bootstrap(). No jQuery at eval, but installed at the model seam with the rest.
 *
 * Globals (frappe, window._version_number, window.dev_server, localStorage) resolve off window. */
/* eslint-disable */
// @ts-nocheck
