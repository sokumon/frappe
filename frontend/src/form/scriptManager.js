// scriptManager.js
//
// Vue-native re-home of the legacy `frappe/form/script_manager.js`. This is the
// client-script pipeline: it defines `frappe.ui.form.on(...)` (which erpnext and
// every doctype `<doctype>.js` calls at load) and `ScriptManager`, whose
// `.trigger(event)` runs the registered handlers. Ported near-verbatim from the
// desk source because the trigger semantics (old-style `doc,cdt,cdn` vs new-style
// `frm,dt,dn`, serial running, immediate `setup`) are subtle and must stay exact
// for old scripts to behave identically.
//
// It lives in the Vue build (not the removed `form_vue_shell` desk bundle) and is
// installed by `installFormEngine()` BEFORE the erpnext bundle loads, so
// `frappe.ui.form.on` exists when erpnext's client scripts evaluate.
//
// Globals (`frappe`, `$`, `cur_frm`, `__`) resolve off `window` — provided by the
// already-loaded libs/list/controls desk bundles.
