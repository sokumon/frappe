// quickEntry.js
//
// Vue-native re-home of the legacy `frappe/form/quick_entry.js`. QuickEntryForm is
// a *dialog*, not a form-layout thing: it `extends frappe.ui.Dialog`, which is
// already the Vue DialogBridge (DialogHost renders it through frappe-ui). So this
// is ported near-verbatim and simply installed AFTER the dialog bridge, capturing
// the bridged Dialog as its base class.
//
// Why it must exist early: erpnext defines
// `frappe.ui.form.ContactAddressQuickEntryForm = class extends frappe.ui.form.QuickEntryForm`
// at bundle-load time — if the base class is missing the erpnext bundle throws and
// the whole app fails to boot. `installQuickEntry()` runs before appendScripts().
//
// Globals (`frappe`, `$`, `cur_frm`, `__`, `is_null`, `locals`) resolve off window.

export function installQuickEntry() {
