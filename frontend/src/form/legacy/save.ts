/* Ported near-verbatim from frappe/public/js/frappe/form/save.js. Wrapped so it
 * runs after the global `frappe` is ready (installed by form/index.ts).
 * Globals (frappe, $, __, cint, cstr, locals) resolve off window. */
/* eslint-disable */
// @ts-nocheck
		...
		$.each(frappe.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
			if (docfield.fieldname) {
				...
				if (df.fieldtype === "Fold") {
					// Vue shell has no legacy layout object (frm.layout is null).
