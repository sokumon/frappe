...
//
// Ported from frappe/public/js/frappe/form/toolbar.js. The side-effect imports
// (linked_with/form_viewers/template_manager) are installed separately by
// form/index.ts; only the ReminderManager ESM import stays. The class body is
// wrapped so it runs after global `frappe` is ready.
/* eslint-disable */
// @ts-nocheck
...
frappe.ui.form.Toolbar = class Toolbar {
	...
	add_open_sidebar() {
		...
		this.page.add_menu_item(
			__("Toggle Sidebar"),
			() => {
				// frm.sidebar is deferred in the Vue shell; guard the toggle handler.
				if (this.frm.sidebar?.sidebar)
